...
RUN_INTERVAL = int(
    os.getenv("SF_TRANSFER_RUN_INTERVAL")
)  # Frequency (in seconds) with which to run bot
CONTINUE_DELAY = int(
    os.getenv("SF_TRANSFER_CONTINUE_DELAY")
)  # Wait (in seconds) before bot runs again after a failed run
MIN_AMOUNT = int(
    os.getenv("SF_TRANSFER_MIN_AMOUNT", 0)
)  # Minimum amount of BYC that must be transferred
...
async def run_stability_fee_transfer_bot():
    ...
    while True:
        ...
        try:
            ...
            print(
                "Failed to list vaults with transferable Stability Fees due to ReadTimeout",
                err,
            )
        ...
        for vault in vaults:
            ...
            try:
                ...
                log.error(
                    "Failed to transfer Stability Fees from vault %s due to ReadTimeout:",
                    vault_name,
                    err,
                )
                failed_transfers += 1
                ...
                log.error(
                    "Failed to transfer Stability Fees from vault %s due to HTTPStatusError: %s",
                    vault_name,
                    str(err),
                )
                failed_transfers += 1
