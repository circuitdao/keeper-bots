...
private_key = os.getenv("PRIVATE_KEY")  # Private master key that controls vault
add_sig_data = str(
...
CONTINUE_DELAY = int(
    os.getenv("LIQUIDATION_START_CONTINUE_DELAY")
)  # Wait (in seconds) before job runs again after a failed run
RUN_INTERVAL = int(
    os.getenv("LIQUIDATION_START_RUN_INTERVAL")
)  # Wait (in seconds) before bot runs again
INITIATOR_PUZZLE_HASH = os.getenv(
    "LIQUIDATION_START_INITIATOR_PUZZLE_HASH"
)  # puzzle hash to which liquidation incentive gets paid
...
if INITIATOR_PUZZLE_HASH not in [None, ""]:
    try:
        bytes32.fromhex(INITIATOR_PUZZLE_HASH)
    except:
        log.exception(
            "Invalid INITIATOR_PUZZLE_HASH. Must be None or convertible to type bytes32"
        )
        raise
...
async def start_liquidation(vault_name, initiator_puzzle_hash, rpc_client):
    ...
    try:
        response = await rpc_client.upkeep_vaults_liquidate(
            vault_name, target_puzzle_hash=initiator_puzzle_hash
        )
    except httpx.ReadTimeout as err:
        log.error(
            "Failed to start liquidation auction for vault %s due to ReadTimeout: %s",
            vault_name,
            str(err),
        )
        raise
        ...
        log.error(
            "Failed to start liquidation auction for vault %s due to ValueError: %s",
            vault_name,
            str(err),
        )
        raise
        ...
        log.error(
            "Failed to start liquidation auction for vault %s: %s", vault_name, str(err)
        )
        raise
...
async def run_liquidation_start_bot():
    if INITIATOR_PUZZLE_HASH in [None, ""]:
        ...
        INNER_PUZZLE_HASH = puzzle_hash_for_synthetic_public_key(
    ...
    while True:
        ...
        if not vaults_pending:
            log.info(
                "No vaults pending liquidation. Sleeping for %s seconds", RUN_INTERVAL
            )
            await asyncio.sleep(RUN_INTERVAL)
            continue

        log.info(
            "%s vaults pending liquidation. Starting liquidation auctions",
            len(vaults_pending),
        )
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                failed_tasks += 1
                log.error(
                    f"Liquidation start task {i} (vault {vaults_pending[i]['name']}) failed: {result}"
                )
            else:
                log.info(
                    f"Liquidation start task {i} (vault {vaults_pending[i]['name']}) succeeded: {result}"
                )
        ...
        log.info(
            "(Re-)started liquidation of all %s liquidatable vaults. Sleeping for %s seconds",
            len(vaults_pending),
            RUN_INTERVAL,
        )
        await asyncio.sleep(RUN_INTERVAL)
