...
async def run_bad_debt_recovery_bot():
    ...
    while True:
        ...
        log.info(
            "%s vaults in bad debt. Starting bad debt recoveries",
            len(vaults_in_bad_debt),
        )
        ...
        for vault in vaults_in_bad_debt:
            try:
                ...
                log.error(
                    "Failed to recover bad debt from vault %s. %s: %s",
                    vault_name,
                    type(err).__name__,
                    err,
                )
        ...
        if num_vaults_not_recovered > 0:
            log.info(
                "Failed to recover bad debt from %s/%s vaults. Sleeping for %s seconds",
                num_vaults_not_recovered,
                len(vaults_in_bad_debt),
                CONTINUE_DELAY,
            )
            await asyncio.sleep(CONTINUE_DELAY)
        ...
        log.info(
            "Recovered some or all bad debt from all %s vaults in bad debt",
            len(vaults_in_bad_debt),
        )
