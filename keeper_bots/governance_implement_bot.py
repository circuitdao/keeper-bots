...
async def run_governance_implement_bot():
    ...
    rpc_url = os.getenv("RPC_URL")  # Base URL for Circuit RPC API server
    private_key = os.getenv("PRIVATE_KEY")  # Private master key that controls announcer
    add_sig_data = os.getenv(
        "ADD_SIG_DATA"
    )  # Additional signature data (depends on network)
    fee_per_cost = os.getenv("FEE_PER_COST")  # Fee per cost for transactions
    RUN_INTERVAL = int(
        os.getenv("IMPLEMENT_RUN_INTERVAL")
    )  # Frequency (in seconds) with which to run bot
    CONTINUE_DELAY = int(
        os.getenv("IMPLEMENT_CONTINUE_DELAY")
    ...
    try:
        while True:
            try:
                ...
                log.error(
                    "Failed to get implementable bills due to ReadTimeout: %s", err
                )
                await asyncio.sleep(CONTINUE_DELAY)
                continue
            except ValueError as err:
                log.error(
                    "Failed to get implementable bills due to ValueError: %s", err
                )
                await asyncio.sleep(CONTINUE_DELAY)
                continue
            except Exception as err:
            ...
            if len(bills) == 0:
                log.info(
                    f"No implementable bills found. Sleeping for {RUN_INTERVAL} seconds"
                )
            ...
            for bill in bills:
                ...
                log.info(
                    ...
                    bill["bill"]["statute_index"],
                    bill["bill"]["statute_name"],
                    bill["bill"]["value"],
                    bill["bill"]["threshold_amount_to_propose"],
                    bill["bill"]["veto_interval"],
                    bill["bill"]["implementation_delay"],
                    bill["bill"]["max_delta"],
                    coin_name,
                )
                try:
                    ...
                    log.error(
                        "Failed to implement bill %s due to ReadTimeout: %s",
                        coin_name,
                        err,
                    )
                    failed_implementations += 1
                    ...
                    log.error(
                        "Failed to implement bill %s due to ValueError: %s",
                        coin_name,
                        err,
                    )
                    failed_implementations += 1
            ...
            if failed_implementations > 0:
                log.info(
                    f"Failed to implement {failed_implementations} of {len(bills)} implementable bills. Sleeping for {CONTINUE_DELAY} seconds"
                )
                await asyncio.sleep(CONTINUE_DELAY)
                continue

            log.info(
                f"Successfully implemented all {len(bills)} implementable bills. Sleeping for {RUN_INTERVAL} seconds"
            )
            await asyncio.sleep(RUN_INTERVAL)
    except asyncio.CancelledError:
        log.info("Shutting down governance implement bot")
    finally:
